...
[source,bash,subs="+quotes,attributes+,+macros"]
----
kubectl -n %USER%-{tutorial-namespace} apply -f mykafka-source.yaml
----
...
kubectl delete -n %USER%-{tutorial-namespace}  -f mykafka-source.yaml
kubectl delete -n %USER%-{tutorial-namespace}  -f eventing-hello-sink.yaml
