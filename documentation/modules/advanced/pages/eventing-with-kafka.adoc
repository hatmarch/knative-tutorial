...
[source,bash,subs="+quotes,attributes+,+macros"]
----
watch "{kubernetes-cli} get pods -n knative-sources"
----
...
releases/download/{kafka-source-version}/kafka-channel.yaml" \
 ...
 | {kubernetes-cli} apply --filename -
----

[NOTE]
=====
* "my-cluster-kafka-bootstrap.kafka:9092" comes from `{kubernetes-cli} get services -n kafka`
...
[source,bash,subs="+quotes,attributes+,+macros"]
----
watch "{kubernetes-cli} get pods -n knative-eventing"
----
...
[source,bash,subs="+quotes,attributes+,+macros"]
----
{kubernetes-cli} api-resources --api-group='sources.knative.dev'
----
--
====
...
[source,bash,subs="+quotes,attributes+,+macros"]
----
{kubernetes-cli} api-resources --api-group='messaging.knative.dev'
----
...
event-display   http://event-display.%USER%-{tutorial-namespace}.example.com   event-display-2mhz8     4h11m   3 OK / 3     True
#eventinghello  http://eventinghello.%USER%-{tutorial-namespace}.example.com   eventinghello-krmch-1   34s     3 OK / 3     True#
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} get -n kafka kafkatopics
----
...
<.> "my-cluster-kafka-bootstrap:9092" can be found via `{kubernetes-cli} get -n kafka services` or `oc get  -n kafka services`
...
[source,bash,subs="+quotes,attributes+,+macros"]
----
{kubernetes-cli} -n %USER%-{tutorial-namespace} apply -f mykafka-source.yaml
----
...
[source,bash,subs="+quotes,attributes+,+macros"]
----
kn sources -n %USER%-{tutorial-namespace} ls
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} -n kafka run kafka-spammer \
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
watch {kubernetes-cli} -n kafka get pods
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} -n kafka exec -it kafka-spammer -- /bin/sh
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} delete -n kafka pod kafka-spammer
{kubernetes-cli} delete -n %USER%-{tutorial-namespace}  -f mykafka-source.yaml
{kubernetes-cli} delete -n %USER%-{tutorial-namespace}  -f eventing-hello-sink.yaml
{kubernetes-cli} delete -n kafka -f kafka-topic-my-topic.yaml
----
