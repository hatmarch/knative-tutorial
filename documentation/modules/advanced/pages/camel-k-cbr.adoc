...
[source,bash,subs="+macros,+attributes"]
----
{kubernetes-cli} apply -n %USER%-{tutorial-namespace} -f $TUTORIAL_HOME/install/utils/fruityvice-proxy.yaml
----
...
[source,bash,subs="+macros,+attributes"]
----
{kubernetes-cli} rollout status deploy fruityvice-proxy
----
...
----
kamel -n %USER%-{tutorial-namespace} run \
 --wait \
...
* `watch {kubernetes-cli} get pods` or `watch oc get pods`
* `watch kamel get`
* `watch {kubernetes-cli} get ksvc`  or `watch oc get ksvc`
====
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} apply -n %USER%-{tutorial-namespace} -f eip/fruits-processor.yaml
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
watch {kubernetes-cli} get  -n %USER%-{tutorial-namespace} camelsources
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} apply -n %USER%-{tutorial-namespace} \
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
minikube -p %USER%-{tutorial-namespace} -n %USER%-{tutorial-namespace} service fruit-events-display
----
--
OpenShift::
+
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
oc expose -n %USER%-{tutorial-namespace} service fruit-events-display
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
oc get -n %USER%-{tutorial-namespace} route fruit-events-display
----
--
====
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} apply -n %USER%-{tutorial-namespace} -f eip/sugary-fruits.yaml
----

Let us check the status of the Trigger using the command `{kubernetes-cli} -n %USER%-{tutorial-namespace} get triggers` which should return one trigger called `sugary-fruits` with ready state as shown below. 

[.console-input]
[#kamel-cbr-list-triggers]
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} -n %USER%-{tutorial-namespace} get triggers
----
...
sugary-fruits  True  default   http://fruits-events-display.%USER%-{tutorial-namespace}.svc.cluster.local/
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
kamel delete  -n %USER%-{tutorial-namespace} fruits-producer
{kubernetes-cli} delete  -n %USER%-{tutorial-namespace} -f eip/fruits-processor.yaml
{kubernetes-cli} delete  -n %USER%-{tutorial-namespace} -f eip/sugary-fruits.yaml
{kubernetes-cli} delete -n %USER%-{tutorial-namespace} -f $TUTORIAL_HOME/install/utils/fruit-events-display.yaml
{kubernetes-cli} delete -n %USER%-{tutorial-namespace} -f $TUTORIAL_HOME/install/utils/fruityvice-proxy.yaml
kn broker delete  -n %USER%-{tutorial-namespace} default
kamel -n %USER%-{tutorial-namespace} reset
kamel -n %USER%-{tutorial-namespace} uninstall
----
