...
-----
kamel install --namespace %USER%-{tutorial-namespace} --olm=false --wait
-----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} create configmap \
  -n %USER%-{tutorial-namespace} my-camel-k-maven-settings \
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} apply -n %USER%-{tutorial-namespace} -f $TUTORIAL_HOME/install/utils/nexus.yaml
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
minikube -p %USER%-{tutorial-namespace} service -n %USER%-{tutorial-namespace} nexus
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
export NEXUS_POD=$({kubernetes-cli} -n %USER%-{tutorial-namespace} get pods \
  -lapp=nexus -ojsonpath='{.items[0].metadata.name}')
{kubernetes-cli} exec $NEXUS_POD -- cat /nexus-data/admin.password
----
