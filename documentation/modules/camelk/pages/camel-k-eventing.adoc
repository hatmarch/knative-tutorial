...
[source,bash,subs="+quotes,+attributes,+macros"]
----
watch '{kubernetes-cli} -n knative-sources get pods'
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
watch {kubernetes-cli} get -n %USER%-{tutorial-namespace} pods
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
kn service -n %USER%-{tutorial-namespace} ls
----
...
event-display   http://event-display.%USER%-{tutorial-namespace}.example.com   event-display-7jvmm   85s   3 OK / 3     True
----
...
event-display   http://event-display.%USER%-{tutorial-namespace}.example.com   event-display-7jvmm   event-display-7jvmm   True
----
--
====
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
{kubernetes-cli} apply -n %USER%-{tutorial-namespace} -f get-started/timed-greeter-source.yaml
----

A successful deployment will show the CamelSource `timed-greeter` in ready state along with its pods in the %USER%-{tutorial-namespace} namespace. 

[.console-input]
[#camel-k-watch-camelsources]
[source,bash,subs="+quotes,+attributes,+macros"]
----
watch {kubernetes-cli} -n %USER%-{tutorial-namespace} get camelsources
----
...
[source,bash,subs="+quotes,+attributes,+macros"]
----
stern -n %USER%-{tutorial-namespace} event-display -c user-container
----
...
event-... user-container   source: camel-source:%USER%-{tutorial-namespace}/timed-greeter
----
...
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete camelsource timed-greeter
----
