...
----
{kubernetes-cli} get trigger helloaloha -o jsonpath='{.status.subscriberUri}'
----
...
[source,bash]
----
http://eventingaloha.%USER%-{tutorial-namespace}.svc.cluster.local
----
...
[source,bash,subs="+macros,+attributes"]
----
{kubernetes-cli} get trigger hellobonjour -o jsonpath='{.status.subscriberUri}'
----
...
[source,bash]
----
http://eventingbonjour.%USER%-{tutorial-namespace}.svc.cluster.local
----
...
[source,bash,subs="+macros,+attributes"]
----
{kubernetes-cli} get broker default -o jsonpath='{.status.address.url}'
----
...
[source,bash]
----
http://broker-ingress.knative-eventing.svc.cluster.local/%USER%-{tutorial-namespace}/default
----

You should notice that the subscriberURIs are Kubernetes services with the suffix of `%USER%-{tutorial-namespace}.svc.cluster.local`.  This means they can be interacted with from another pod within the Kubernetes cluster.
...
[source,bash,subs="+macros,+attributes"]
-----
{kubernetes-cli} -n %USER%-{tutorial-namespace} apply -f curler.yaml
-----
...
[source,bash,subs="+macros,+attributes"]
----
{kubernetes-cli} -n %USER%-{tutorial-namespace} exec -it curler -- /bin/bash
----
...
[source,bash,subs="+macros,+attributes"]
----
curl -v "http://eventingaloha.%USER%-{tutorial-namespace}.svc.cluster.local" \
-X POST \
-H "Ce-Id: say-hello" \
...
[source,bash,subs="+macros,+attributes"]
----
curl -v "http://eventingbonjour.%USER%-{tutorial-namespace}.svc.cluster.local" \
-X POST \
-H "Ce-Id: say-hello" \
-H "Ce-Specversion: 1.0" \
-H "Ce-Type: bonjour" \
...
[source,bash,subs="+macros,+attributes"]
----
curl -v "http://broker-ingress.knative-eventing.svc.cluster.local/%USER%-{tutorial-namespace}/default" \
-X POST \
-H "Ce-Id: say-hello" \
-H "Ce-Specversion: 1.0" \
-H "Ce-Type: greeting" \
...
[source,bash,subs="+macros,+attributes"]
----
kn service delete eventingbonjour -n %USER%-{tutorial-namespace}
kn service delete eventingaloha -n %USER%-{tutorial-namespace}
kn trigger delete helloaloha -n %USER%-{tutorial-namespace}
kn trigger delete hellobonjour -n %USER%-{tutorial-namespace}
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete pod curler
kn broker delete default -n %USER%-{tutorial-namespace}
----
--
{kubernetes-cli}::
+
...
[source,bash,subs="+macros,+attributes"]
----
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete -f eventing-aloha-sink.yaml
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete -f eventing-bonjour-sink.yaml
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete -f trigger-helloaloha.yaml
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete -f trigger-hellobonjour.yaml
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete -f curler.yaml
{kubernetes-cli} -n %USER%-{tutorial-namespace} delete -f default-broker.yaml
----
--
====
